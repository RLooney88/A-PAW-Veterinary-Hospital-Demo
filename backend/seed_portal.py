...
async def seed_portal():
    async with AsyncSessionLocal() as db:
        ...
        res = await db.execute(select(Client).where(Client.email == "roddylooney@gmail.com"))
        ...
        # Contacts for Rosie
        db.add(PetContact(pet_id=rosie.id, name="Roddy Looney", relation="owner", phone="(410) 555-0199", email="roddylooney@gmail.com"))
        db.add(PetContact(pet_id=rosie.id, name="Emergency Vet (Anne Arundel)", relation="emergency", phone="(410) 224-0331"))
        ...
        # Appointments
        db.add(PetAppointment(pet_id=rosie.id, date="2025-06-10", reason="Annual Wellness Exam + Vaccines", provider="Dr. Karen Hamilton", status="completed", notes="All vaccines updated. Weight stable. Heart and lungs clear."))
        db.add(PetAppointment(pet_id=rosie.id, date="2025-09-18", reason="Bordetella Booster", provider="Dr. Karen Hamilton", status="completed", notes="Quick visit. No concerns."))
        db.add(PetAppointment(pet_id=rosie.id, date="2024-11-05", reason="Dental Cleaning", provider="Dr. Karen Hamilton", status="completed", notes="Dental cleaning under anesthesia. Recovered well."))
        db.add(PetAppointment(pet_id=rosie.id, date="2026-06-10", reason="Annual Wellness Exam", provider="Dr. Karen Hamilton", status="upcoming"))

        await db.commit()
        logger.info("Portal seed: created Roddy Looney + Rosie the Chizon with full health records.")
